...
class PacketTime(DataTime):
    ...
    def __eq__(self, other) -> bool:
        return type(other) == PacketTime and other.second_precision == self.__second_precision and other.microseconds == self.__microseconds
    
    def __hash__(self) -> int:
        return hash((self.__second_precision, self.__microseconds))
    
    def __str__(self) -> str:
        return time.strftime("%Y-%m-%d %H:%M:%S", self.__second_precision) + ".{:06d}".format(self.__microseconds)