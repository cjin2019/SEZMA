...
def calculate_mscn(dis_image):
    ...
    ux = cv2.GaussianBlur(dis_image, (7, 7), 7/6) # old values
    ux_sq = ux*ux # old code
...
def segmentEdge(blockEdge, nSegments, blockSize, windowSize):
    ...
    for i in range(nSegments):
        segments[i, :] = blockEdge[i:i+windowSize]
        # old code
...
def piqe(im):
    ...
    for i in np.arange(0, imnorm.shape[0]-1, blockSize):
        for j in np.arange(0, imnorm.shape[1]-1, blockSize):
            ...
            if(blockVar >= activityThreshold):
                ...
                # code added
                # if WNC == 1 and WNDC == 1:
                #     BlockScores.append(1)
                # elif WNC == 1:
                #     BlockScores.append(blockVar)
                # elif WNDC == 1:
                #     BlockScores.append(1 - blockVar)
                # old code already commented out
    ...
    # Score = (sum(BlockScores) + C) / (NHSA + C) # new code
